...
Base.:(==)(c1::ContinuedFraction{T}, c2::ContinuedFraction{T}) where T<:Integer = Rational(c1)==Rational(c2)
...
function CF(x...)
  ContinuedFraction(x...) |> positive_form
end
...
function Base.show(io::IO, c::ContinuedFraction{T}) where T<:Integer
  return print(io, "$(Rational(c)): " * cf_string(c))
end


function Base.show(io::IO, mime::MIME"text/plain", c::ContinuedFraction{T}) where T<:Integer
  println(io, repr(typeof(c)))
  return print(io, cf_string(c) * " = $(Rational(c))")
end


function cf_string(c::ContinuedFraction{T}) where T<:Integer
  listvals = isinteger(c) ? "]" : split("$(c[2:end])", "[")[end]
  repr = "[$(c.leading); " * listvals
end
